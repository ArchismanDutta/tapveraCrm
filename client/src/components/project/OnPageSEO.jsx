...
const OnPageSEO = ({ projectId, userRole, userId }) => {
  ...
  const getTrendIcon = (trend) => {
    ...
    return <Minus className="w-4 h-4 text-slate-400 dark:text-gray-400" />;
  };
  ...
  const getRankBadge = (rankObj) => {
    if (!rankObj) return <span className="text-slate-400 dark:text-gray-600 text-sm">—</span>;
    const r = rankObj.rank;
    if (r === 0 || r >= 101) {
      return (
        <span className="inline-flex items-center justify-center w-12 h-8 rounded-md bg-slate-100 border border-slate-200 text-slate-500 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-500 text-xs font-bold">N/R</span>
      );
    }
  ...
  return (
    <div className="p-4 sm:p-6 space-y-6">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          {canEdit && undoStack.length > 0 && (
            <button
              onClick={handleUndo}
              className="flex items-center gap-1.5 px-3 py-1.5 bg-[#1a1f2e] hover:bg-slate-100 dark:hover:bg-[#232945] border border-slate-200 dark:border-[#232945] text-slate-400 dark:text-gray-400 rounded-lg transition-all text-sm"
      ...
      {/* Velocity Metrics */}
      {keywords.length > 0 && (
        <div className="border border-slate-200 bg-white dark:border-[#232945] dark:bg-[#0f1419] rounded-xl overflow-hidden">
          {/* Collapsible Header */}
          <div
            className="flex items-center justify-between p-4 cursor-pointer hover:bg-slate-50 dark:hover:bg-[#1a1f2e] transition-colors"
            onClick={() => setShowVelocityMetrics(!showVelocityMetrics)}
          >
            <div className="flex items-center gap-2">
              <TrendingUp className="w-5 h-5 text-orange-400" />
              <h4 className="text-lg font-bold text-slate-950 dark:text-white">Velocity Metrics</h4>
              <span className="text-xs text-slate-400 dark:text-gray-400 ml-2">
                (Click to {showVelocityMetrics ? 'collapse' : 'expand'})
              </span>
            </div>
            <button
              type="button"
              className="p-2 hover:bg-slate-100 dark:hover:bg-[#232945] rounded-lg transition-colors"
              onClick={(e) => {
                e.stopPropagation();
            ...
            >
              {showVelocityMetrics ? (
                <ChevronUp className="w-5 h-5 text-slate-400 dark:text-gray-400" />
              ) : (
          ...
          >
            <div className="p-6 pt-0 border-t border-slate-200 dark:border-[#232945]">
      ...
      {selectedDate && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70">
          <div className="rounded-xl border border-slate-200 bg-white shadow-2xl dark:border-[#232945] dark:bg-[#191f2b] w-full max-w-2xl max-h-[80vh] overflow-y-auto">
            <div className="p-6">
              ...
              <div className="grid grid-cols-3 gap-4 mb-6">
                <div className="border border-slate-200 bg-white dark:border-[#232945] dark:bg-[#0f1419] rounded-lg p-4 text-center">
                  <div className="text-sm text-slate-400 dark:text-gray-400 mb-1">Total Updates</div>
                  <div className="text-2xl font-bold text-slate-950 dark:text-white">{selectedDate.data.updates}</div>
                </div>
                <div className="border border-green-200 bg-white dark:border-green-500/30 dark:bg-[#0f1419] rounded-lg p-4 text-center">
                  <div className="text-sm text-slate-400 dark:text-gray-400 mb-1">Improved</div>
                  <div className="text-2xl font-bold text-green-400">{selectedDate.data.improvements}</div>
                </div>
                <div className="border border-red-200 bg-white dark:border-red-500/30 dark:bg-[#0f1419] rounded-lg p-4 text-center">
                  <div className="text-sm text-slate-400 dark:text-gray-400 mb-1">Declined</div>
              ...
              <div className="space-y-2">
                {selectedDate.data.keywords.map((kw, idx) => (
                  <div
                    key={idx}
                    className="border border-slate-200 bg-white dark:border-[#232945] dark:bg-[#0f1419] rounded-lg p-4 hover:border-purple-500/50 transition-colors"
                  >
                    <div className="flex items-center justify-between">
                      <div>
                        <div className="font-medium text-slate-950 dark:text-white">{kw.keyword}</div>
                        <div className="text-sm text-slate-400 dark:text-gray-400 mt-1">
                      ...
                      <div className="text-right">
                        <div className={`text-2xl font-bold ${
                          ...
                          kw.change < 0 ? 'text-red-400' : 'text-slate-400 dark:text-gray-400'
                        }`}>
                          {kw.change > 0 ? '+' : ''}{kw.change}
                        </div>
                        <div className="text-xs text-slate-500 dark:text-gray-500 mt-1 flex items-center justify-end gap-1">
                          {kw.change > 0 ? (
      ...
      {loading ? (
        <div className="flex flex-col items-center justify-center h-48 gap-3">
          ...
          <p className="text-sm text-slate-500 dark:text-gray-500">Loading keywords...</p>
        </div>
      ) : keywords.length === 0 ? (
        <div className="bg-[#0f1419] border border-dashed border-slate-300 dark:border-[#232945] rounded-xl p-12 text-center">
          <Search className="w-10 h-10 text-slate-300 dark:text-gray-700 mx-auto mb-3" />
          <p className="text-slate-400 dark:text-gray-400 font-medium">No keywords tracked yet</p>
          {canEdit && (
            <p className="text-sm text-slate-400 dark:text-gray-600 mt-1">Click "Add Keyword" to start tracking</p>
          )}
        </div>
      ) : (
        <div className="border border-slate-200 bg-white dark:border-[#232945] dark:bg-[#0f1419] rounded-xl overflow-hidden">
          {/* Search + Filter bar */}
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 px-4 py-3 border-b border-slate-200 dark:border-[#232945] bg-slate-50 dark:bg-[#0c1117]">
            <div className="relative flex-1">
              ...
              <input
                ...
                className="w-full rounded-lg border border-slate-200 bg-white py-1.5 pl-9 pr-3 text-sm text-slate-900 placeholder-slate-400 focus:border-blue-500/50 focus:outline-none dark:border-[#232945] dark:bg-[#141a21] dark:text-white dark:placeholder-gray-600"
              />
            </div>
            <div className="flex items-center gap-1 bg-slate-50 dark:bg-[#141a21] border border-slate-200 dark:border-[#232945] rounded-lg p-1">
              {["all", "SEO", "GMB"].map((cat) => (
                <button
                  key={cat}
            ...
            <span className="text-xs text-slate-400 dark:text-gray-600 px-1 whitespace-nowrap">
            ...
            <table className="w-full">
                  ...
                  {canEdit && (
                    <th className="pl-4 py-3 w-10">
                      <button
                        ...
                        className="p-1 hover:bg-slate-100 dark:hover:bg-[#232945] rounded transition-colors"
                        title={selectedKeywords.length === keywords.length ? "Deselect All" : "Select All"}
                      >
                        {selectedKeywords.length === keywords.length ? (
                          <CheckSquare className="w-4 h-4 text-blue-400" />
                        ) : (
                          <Square className="w-4 h-4 text-slate-400 dark:text-gray-600" />
                        )}
                      </button>
                    </th>
                  )}
              ...
              <tbody className="divide-y divide-slate-100 dark:divide-[#1a1f2e]">
                {filteredKeywords.length === 0 ? (
                  <tr>
                    <td colSpan={canEdit ? 8 : 7} className="text-center py-10 text-sm text-slate-400 dark:text-gray-600">
                  ...
                  <tr
                    key={keyword._id}
                    className={`hover:bg-slate-50 dark:hover:bg-[#0f1623] transition-colors ${selectedKeywords.includes(keyword._id) ? "bg-blue-500/5" : ""}`}
                  >
                    {canEdit && (
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-start gap-2">
                        ...
                        <div>
                          <div className="flex items-center gap-2 flex-wrap">
                            <span className="font-medium text-slate-950 dark:text-white text-sm">{keyword.keyword}</span>
                          ...
                          <div className="flex items-center gap-1.5 mt-0.5 text-xs text-slate-400 dark:text-gray-600 flex-wrap">
                    ...
                    <td className="px-4 py-3 text-center">
                      <div className="flex flex-col items-center gap-0.5">
                        ...
                        {keyword.pastRank && (
                          <span className="text-xs text-slate-300 dark:text-gray-700">
                    ...
                    <td className="px-4 py-3 text-center">
                      {(() => {
                        ...
                        if (!keyword.previousRank) return <span className="text-slate-300 dark:text-gray-700 text-xs">—</span>;
                        return (
                          <span className={`inline-flex items-center gap-0.5 px-2 py-0.5 rounded-full text-xs font-semibold border ${
                            ...
                            "bg-gray-500/15 text-slate-500 dark:text-gray-500 border-gray-600/30"
                          }`}>
                        ...
                        {canEdit && (
                          <>
                            <button
                              ...
                              className="p-1.5 text-slate-500 dark:text-gray-500 hover:text-yellow-400 hover:bg-yellow-500/10 rounded transition-colors"
                              title="Edit keyword"
                            >
                            ...
                            <button
                              ...
                              className={`p-1.5 rounded transition-colors ${
                                fetchingRankId === keyword._id || keyword.searchEngine !== "Google" || !keyword.targetUrl
                                  ...
                                  : "text-slate-500 dark:text-gray-500 hover:text-purple-400 hover:bg-purple-500/10"
                              }`}
                              title={
                            ...
                            <button
                              ...
                              className="p-1.5 text-slate-500 dark:text-gray-500 hover:text-green-400 hover:bg-green-500/10 rounded transition-colors"
                            ...
                            <button
                              ...
                              className="p-1.5 text-slate-400 dark:text-gray-600 hover:text-red-400 hover:bg-red-500/10 rounded transition-colors"
      ...
      {showAddModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70">
          <div className="rounded-xl border border-slate-200 bg-white shadow-2xl dark:border-[#232945] dark:bg-[#191f2b] w-full max-w-md max-h-[90vh] overflow-y-auto">
            <div className="p-6">
              <div className="flex items-center justify-between mb-6">
                <h3 className="text-xl font-bold text-slate-950 dark:text-white">Add Keyword</h3>
                <button
                  onClick={() => {
                    setShowAddModal(false);
                    resetForm();
              ...
              <form onSubmit={handleAddKeyword} className="space-y-4">
                ...
                <div>
                  ...
                  />
                  <p className="text-xs text-slate-500 dark:text-gray-500 mt-1">Domain to find in Google results</p>
                </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block text-sm font-medium text-slate-700 dark:text-gray-300 mb-2">
                      City <span className="text-slate-500 dark:text-gray-500 text-xs font-normal">(for local SEO)</span>
                    </label>
                    <input
                      type="text"
                      value={formData.city}
                ...
                <div className="flex gap-3 pt-4">
                  <button
                    ...
                    className="flex-1 rounded-lg border border-slate-200 bg-white px-4 py-2 text-slate-900 transition-colors hover:bg-slate-50 dark:border-[#232945] dark:bg-[#0f1419] dark:text-white dark:hover:bg-slate-50 dark:hover:bg-slate-50 dark:bg-[#141a21]"
                  >
                    Cancel
                  </button>
                  <button
      ...
      {showUpdateModal && selectedKeyword && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70">
          <div className="rounded-xl border border-slate-200 bg-white shadow-2xl dark:border-[#232945] dark:bg-[#191f2b] w-full max-w-md">
            <div className="p-6">
              <div className="flex items-center justify-between mb-6">
                <h3 className="text-xl font-bold text-slate-950 dark:text-white">
              ...
              <form onSubmit={handleUpdateRank} className="space-y-4">
                <div className="border border-slate-200 bg-white dark:border-[#232945] dark:bg-[#0f1419] rounded-lg p-4 mb-4">
                  <div className="grid grid-cols-3 gap-4 text-center">
                    <div>
                      <div className="text-xs text-slate-500 dark:text-gray-500">Past</div>
                      <div className="text-lg text-slate-400 dark:text-gray-400 font-semibold mt-1">
                        {selectedKeyword.pastRank?.rank || "-"}
                      </div>
                    </div>
                    <div>
                      <div className="text-xs text-slate-500 dark:text-gray-500">Previous</div>
                      <div className="text-lg text-slate-400 dark:text-gray-400 font-semibold mt-1">
                        {selectedKeyword.previousRank?.rank || "-"}
                      </div>
                    </div>
                    <div>
                      <div className="text-xs text-slate-500 dark:text-gray-500">Current</div>
                      <div className="text-lg text-slate-950 dark:text-white font-bold mt-1">
      ...
      {showEditModal && selectedKeyword && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70">
          <div className="rounded-xl border border-slate-200 bg-white shadow-2xl dark:border-[#232945] dark:bg-[#191f2b] w-full max-w-md max-h-[90vh] overflow-y-auto">
            <div className="p-6">
              <div className="flex items-center justify-between mb-6">
                <h3 className="text-xl font-bold text-slate-950 dark:text-white">Edit Keyword</h3>
                <button
                  onClick={() => {
                    setShowEditModal(false);
                    setSelectedKeyword(null);
      ...
      {showChartModal && selectedKeyword && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70">
          <div className="rounded-xl border border-slate-200 bg-white shadow-2xl dark:border-[#232945] dark:bg-[#191f2b] w-full max-w-4xl max-h-[90vh] overflow-y-auto">
            <div className="p-6">
              <div className="flex items-center justify-between mb-6">
                <div>
                  <h3 className="text-xl font-bold text-slate-950 dark:text-white">
              ...
              <div className="border border-slate-200 bg-white dark:border-[#232945] dark:bg-[#0f1419] rounded-lg p-4" style={{ height: "400px" }}>
                <Line data={getChartData(selectedKeyword)} options={chartOptions} />
              </div>

              <div className="mt-6 border border-slate-200 bg-white dark:border-[#232945] dark:bg-[#0f1419] rounded-lg p-4">
                ...
                <div className="space-y-2 max-h-48 overflow-y-auto">
                  {[...selectedKeyword.rankHistory].reverse().map((record, index) => (
                    <div
                      key={index}
                      className="flex items-center justify-between py-2 px-3 bg-slate-50 dark:bg-[#141a21] rounded-lg"
                    >
                      <div className="flex items-center gap-3">
                        <div className={`text-2xl font-bold ${record.rank >= 101 || record.rank === 0 ? "text-red-400 text-base" : "text-slate-950 dark:text-white"}`}>
                        ...
                        <div>
                          ...
                          {record.notes && (
                            <div className="text-xs text-slate-400 dark:text-gray-400 mt-0.5">{record.notes}</div>
                          )}
                        </div>
                      </div>
                      <div className="flex flex-col items-end gap-1">
                        <span className={`text-xs px-1.5 py-0.5 rounded-full ${
                          ...
                          record.source === "scrape" ? "bg-orange-500/20 text-orange-300" :
                                                       "bg-gray-500/20   text-slate-500 dark:text-gray-500"
                        }`}>
      ...
      <div className="mt-12 pt-8 border-t border-slate-200 dark:border-[#232945]">
        <BlogUpdates
          projectId={projectId}
          userRole={userRole}
          userId={userId}
